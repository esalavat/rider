...
This was the fourth pacing-tuning pass in a row, and correctly predicted that a fifth might be needed if a single flat soft-cap threshold couldn't serve both ends — see "Round five" immediately below for exactly that resurfacing.

**Round five: the 2,000,000 cap fixed Chapter 4 but silently left Vice President just as exposed as Patched Member used to be, and the flattened Chapter 4-8 curve was never revisited after the cap changed.** Real playtest after round four: Chapters 1-3 felt right, but the moment Pine Ridge unlocked Sergeant at Arms and a play session ran for a while, Legend Points exploded past 15,000,000 — enough to charter every remaining chapter (4 through 8) in one sitting. Root cause, confirmed with a standalone numeric simulation of the full economy (rackets, member tiers, prestige, chapter costs — not just reasoning about the formulas): the 2,000,000 cap was sized around Sergeant (weight 8,000) but Vice President's weight (160,000) is *also* comfortably under 2,000,000 — over 12 VPs' worth of completely uncapped linear scaling — so the instant Bayou Crossing's tier unlocked, the same "cheap tier, huge weight jump, cash windfall buys a burst of them" pattern that hit Patched Member pre-round-four hit Vice President instead, just one tier later. Compounding it: Chapters 4-8's cost curve (200,000 / 500,000 / 1,200,000 / 3,000,000 / 7,500,000, a flat ~2.4-3.3x) was deliberately softened in round four to escape a *low-income* wall caused by the old 200-value cap — but nobody revisited that curve once the cap was fixed to 2,000,000 and real income headroom came back, so it stayed priced for a starved economy that no longer existed. Fixed two ways, validated against the simulation before shipping (chapter-by-chapter unlock times now climb steadily — no single prestige clears more than two chapters, no chapter takes more than a few days even at the very end):
1. **`MEMBER_WEIGHT_SOFT_CAP` lowered from 2,000,000 to 500,000.** Tiers 1-3 keep hundreds-to-thousands of headroom, Sergeant still gets a few dozen units before throttling (down from "hundreds," a deliberate tightening), and Vice President now hits the log climb within single-digit units instead of a dozen-plus — closing exactly the gap round four left open. Chapter President/National President (weight 3.2M/64M) were already far enough past any reasonable cap that they're unaffected either way; they were never the source of this runaway.
2. **Bayou Crossing's cost raised from 200,000 to 1,200,000 LP** — restoring the same ~7.5x cost-to-weight ratio validated at Salt Flats and Pine Ridge (both `cost / tier weight = 7.5`), instead of the collapsed ~1.25x ratio it had drifted to. **Copper Canyon (500,000 → 1,500,000) and Steel Harbor (1,200,000 → 2,000,000)** got smaller bumps too, mostly so the map's costs stay monotonically increasing and so Copper Canyon fully decouples from clearing alongside Bayou Crossing in the same sitting — simulation showed pricing them proportional to raw weight (like Bayou Crossing) recreates a multi-day-to-multi-week wall instead, because Chapter President/National President are already log-throttled from their first unit and can't fuel the kind of income spike that would justify it. **Wildfire Mesa and Vulture's Rest are unchanged** — same reasoning, no tier left to unlock so no fresh spike to chase, and their existing growth off the new Steel Harbor cost already gave enough runway in simulation.

**If a sixth round is ever needed:** don't re-guess from the formulas — write (or reuse) a small standalone simulation of the economy first. Reasoning about interacting exponential systems (member weight, racket cost growth, `sqrt` LP conversion, repeated prestige) by hand is exactly how rounds one through four each under- or over-corrected; a simulation turns "does this chapter curve create a wall" from a multi-day real playtest into a few seconds of iteration.
...
| 4 | Bayou Crossing | Deep South | Vice President | 1,200,000 | 20x | +5% | moon | `#4a8a94` |
| 5 | Copper Canyon | Southwest Mesa | Chapter President | 1,500,000 | 1.25x | +5% | mesa | `#b5651d` |
| 6 | Steel Harbor | Coastal Port | National President | 2,000,000 | 1.33x | +5% | anchor | `#7d8c99` |
| 7 | Wildfire Mesa | High Desert Badlands | — (income only) | 3,000,000 | 1.5x | +20% | flame | `#a3222b` (blood) |
| 8 | Vulture's Rest | National Chapter | — (income only) | 7,500,000 | 2.5x | +30% | wings | `#8a5aa0` |

**Fourth-pass numbers.** Chapters 1-3 (30 / 3,000 / 60,000 LP, ratios 100x then 20x) are unchanged and validated good across multiple rounds of real play — a satisfying ~20-30 min, then +20 min, then +~1 hour. Chapters 4-8 used to keep **accelerating** (25x, 30x, 35x, 40x, 50x, topping out at 3,150,000,000,000 LP) on the theory that the game should keep getting harder to advance in at a compounding rate. In practice this produced a real wall: Chapter 4 took **over 24 hours** and needed dev tools to even reach, because the member-weight soft cap (see "Structural fix" above) had — independently — already throttled recruit income to near-zero growth by that point, so there was no realistic way to out-earn a 25x-and-climbing cost ratio. Both problems are fixed together: the soft-cap threshold fix restores meaningful recruit scaling much further into the game, and this curve was changed from *accelerating* to roughly **constant** (~2.4-3.3x per step, closer to the "each chapter roughly doubles the time" pacing actually requested) — gentler than before by a wide margin, but still requiring real sustained investment, not a five-minute clear. Total LP to charter all 8 was ~12.5M at the time, down from ~3.15T. Chapters 7-8 keep their raised +20%/+30% bonus (up from the old flat +5%/+10%) so they still feel worth their cost.

**Fifth-pass: Bayou Crossing, Copper Canyon, and Steel Harbor all repriced upward; Wildfire Mesa and Vulture's Rest left alone.** The fourth-pass curve above was tuned around a starved economy (the miscalibrated 200-value soft cap) and, once that cap was corrected to 2,000,000, turned out to badly underprice everything from Bayou Crossing on — see "Round five" under "Structural fix" above for the full diagnosis (Vice President's weight sits under even the old 2,000,000 cap, so it inherited Patched Member's old "cheap tier, huge weight jump" runaway one tier later). Fixed via a standalone simulation of the full economy rather than by feel:
- **Bayou Crossing: 200,000 → 1,200,000 LP** (6x) — restores the ~7.5x cost-to-weight ratio validated at Salt Flats and Pine Ridge, instead of the collapsed ~1.25x ratio it had drifted to.
- **Copper Canyon: 500,000 → 1,500,000 LP** (3x) and **Steel Harbor: 1,200,000 → 2,000,000 LP** (1.7x) — smaller bumps, mainly to keep the cost curve monotonically increasing chapter-over-chapter (a chapter should never look *cheaper* than the one before it on the map) and to fully decouple Copper Canyon from clearing in the same sitting as Bayou Crossing. These tiers (Chapter President/National President, weight 3.2M/64M) are already so far past any workable soft-cap value that pricing them much higher than this doesn't track a real income-growth opportunity — simulation confirmed that repricing them proportional to their raw weight (matching Bayou Crossing's treatment) instead creates a multi-day-to-multi-week wall, since those tiers are log-throttled from their very first unit and can't fuel the kind of spike that would justify it.
- **Wildfire Mesa and Vulture's Rest: unchanged.** Same reasoning — no tier to unlock, so no fresh income spike to chase; their existing gentle ~1.5-2.5x-ish growth off the new Steel Harbor cost was already enough runway in simulation.

Total LP to charter all 8 is now ~15.3M, up from ~12.5M.
...
- **Chapter costs**: went through five real-play/simulation-driven rebalance rounds (see "Notable fixes" in `ROADMAP.md`) — considered validated for now, revisit only if a future playtest surfaces a new wall or runaway.
